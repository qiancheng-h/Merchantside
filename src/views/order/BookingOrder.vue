...
import { bookingOrders, statusMap } from '@/mock/orderData'
...
const data = ref<OrderItem[]>([...bookingOrders])
...
const columns: TableColumnsType = [
  ...
  { title: '服务名称', dataIndex: 'serviceName', key: 'serviceName', width: 180, ellipsis: true },
  { title: '状态', dataIndex: 'status', key: 'status', width: 100 },
  { title: '预约日期', dataIndex: 'appointDate', key: 'appointDate', width: 110 },
  { title: '预约时段', dataIndex: 'appointTime', key: 'appointTime', width: 120 },
  { title: '服务地点', dataIndex: 'serviceLocation', key: 'serviceLocation', width: 140, ellipsis: true },
  ...
  Modal.confirm({
    ...
    onOk() {
      ...
      if (idx !== -1) {
        ...
        data.value[idx] = { ...data.value[idx]!, status: 'PENDING_USE', confirmTime: now,
          flowRecords: [...data.value[idx]!.flowRecords, { id: `f${Date.now()}`, time: now, desc: '商家确认订单', operator: '商家账号' }] } as OrderItem
...
const handleReject = (order: OrderItem) => {
  ...
  if (idx !== -1) {
    const now = new Date().toISOString().replace('T', ' ').substring(0, 19)
    data.value[idx] = { ...data.value[idx]!, status: 'CANCELLED', cancelTime: now,
      flowRecords: [...data.value[idx]!.flowRecords, { id: `f${Date.now()}`, time: now, desc: '商家不可接待，订单取消', operator: '商家账号' }] } as OrderItem
  }
}
...
const handleSupplementSuccess = (newOrder: any) => { data.value.unshift({ ...newOrder, orderType: 'booking' }) }
const handleImportSuccess = (count: number) => { message.success(`已导入 ${count} 条预定服务订单`) }
...
<template>
  <div>
    <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 20px;">
      <CalendarOutlined style="font-size: 20px; color: #006738;" />
      <h2 style="font-size: 20px; font-weight: 700; color: #262626; margin: 0;">预定服务订单</h2>
    </div>
    ...
    <a-card :bordered="false" style="margin-bottom: 16px;">
      <a-form layout="inline" :model="searchForm">
        ...
        <a-form-item label="服务名称">
          <a-input v-model:value="searchForm.serviceName" placeholder="请输入服务名称" allow-clear style="width: 160px;" />
        </a-form-item>
        ...
        <template v-if="column.key === 'action'">
          <a-space>
            <template v-if="record.status === 'PENDING_CONFIRM'">
              <a-button type="link" size="small" style="color: #006738;" @click="handleConfirm(record)">确认订单</a-button>
              <a-button type="link" size="small" danger @click="handleReject(record)">不可接待</a-button>
            </template>
    ...
    />
    <SupplementModal v-model:open="supplementOpen" default-type="booking" @success="handleSupplementSuccess" />
    <ImportModal v-model:open="importOpen" order-type="booking" @success="handleImportSuccess" />
  </div>
</template>
